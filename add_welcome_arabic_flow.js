...
async function addWelcomeArabicFlow() {
  const { data: flows } = await supabase.from('flows').select('id').eq('name', 'Welcome Inbound Flow').limit(1);
  if (!flows || flows.length === 0) {
    console.error("Welcome Inbound Flow not found");
    return;
  }
  ...
  const nodes = [
    {
      id: uuid(), flow_id: flowId, node_key: 'ask_customer_type_ar', node_type: 'send_buttons',
      config: {
        text: 'مرحباً! أهلاً بك في Nexvora. شكراً لتواصلك معنا.\n\nلمساعدتك بشكل أفضل، هل أنت عميل جديد أم عميل حالي؟',
        buttons: [
          { reply_id: 'new_cust_ar', title: '🆕 عميل جديد', next_node_key: 'ask_source_ar' },
          { reply_id: 'exist_cust_ar', title: '👤 عميل حالي', next_node_key: 'ask_existing_help_ar' }
        ]
      }, position_x: 600, position_y: -600
    },
    // Existing Customer (Arabic)
    {
      id: uuid(), flow_id: flowId, node_key: 'ask_existing_help_ar', node_type: 'send_list',
      config: {
        text: 'أهلاً بك مجدداً في Nexvora! 👋 يسعدنا رؤيتك مرة أخرى.\n\nكيف يمكننا مساعدتك اليوم؟ يرجى اختيار أحد الخيارات أدناه.',
        button_label: 'اختر الخيار',
        sections: [{ title: 'خيارات المساعدة', rows: [
          { reply_id: 'exist_chg_ar', title: '🌐 تعديل على الموقع', next_node_key: 'msg_exist_changes_ar' },
          { reply_id: 'exist_sup_ar', title: '🛠️ دعم فني', next_node_key: 'msg_exist_support_ar' },
          { reply_id: 'exist_sts_ar', title: '⏳ حالة المشروع', next_node_key: 'msg_exist_status_ar' },
          { reply_id: 'exist_con_ar', title: '💼 استشارة أعمال', next_node_key: 'msg_exist_consult_ar' },
          { reply_id: 'exist_tlk_ar', title: '💬 التحدث مع فريقنا', next_node_key: 'msg_exist_talk_ar' }
        ]}]
      }, position_x: 900, position_y: -800
    },
    {
      id: uuid(), flow_id: flowId, node_key: 'msg_exist_changes_ar', node_type: 'send_message',
      config: { text: "أنا مساعدك الذكي. يرجى توضيح التعديلات المحددة التي ترغب في إجرائها على موقعك؟", next_node_key: 'handoff_end' },
      position_x: 1200, position_y: -900
    },
    {
      id: uuid(), flow_id: flowId, node_key: 'msg_exist_support_ar', node_type: 'send_message',
      config: { text: "أنا مساعدك الذكي. ما هي المشكلة الفنية أو المساعدة التي تحتاجها؟", next_node_key: 'handoff_end' },
      position_x: 1200, position_y: -800
    },
    {
      id: uuid(), flow_id: flowId, node_key: 'msg_exist_status_ar', node_type: 'send_message',
      config: { text: "أنا مساعدك الذكي. ما الذي ترغب في معرفته بشأن حالة مشروعك؟", next_node_key: 'handoff_end' },
      position_x: 1200, position_y: -700
    },
    {
      id: uuid(), flow_id: flowId, node_key: 'msg_exist_consult_ar', node_type: 'send_message',
      config: { text: "أنا مساعدك الذكي. ما هو نوع استشارة الأعمال التي تبحث عنها؟", next_node_key: 'handoff_end' },
      position_x: 1200, position_y: -600
    },
    {
      id: uuid(), flow_id: flowId, node_key: 'msg_exist_talk_ar', node_type: 'send_message',
      config: { text: "أنا مساعدك الذكي. يرجى كتابة استفسارك أو مشكلتك، وسيقوم فريقنا بالرد عليك في أقرب وقت ممكن.", next_node_key: 'handoff_end' },
      position_x: 1200, position_y: -500
    },

    // New Customer Source (Arabic)
    {
      id: uuid(), flow_id: flowId, node_key: 'ask_source_ar', node_type: 'send_list',
      config: {
        text: 'شكراً لاختيارك Nexvora! سنكون سعداء بمعرفة كيف سمعت عنا؟',
        button_label: 'اختر المصدر',
        sections: [{ title: 'المصادر (Sources)', rows: [
          { reply_id: 'src_fb_ar', title: '📱 فيسبوك (Facebook)', next_node_key: 'ask_city_ar' },
          { reply_id: 'src_ig_ar', title: '📸 إنستغرام (Instagram)', next_node_key: 'ask_city_ar' },
          { reply_id: 'src_wa_ar', title: '💬 واتساب (WhatsApp)', next_node_key: 'ask_city_ar' },
          { reply_id: 'src_gg_ar', title: '🔍 بحث جوجل (Google)', next_node_key: 'ask_city_ar' },
          { reply_id: 'src_yt_ar', title: '🎥 يوتيوب (YouTube)', next_node_key: 'ask_city_ar' },
          { reply_id: 'src_ref_ar', title: '👥 توصية صديق', next_node_key: 'ask_city_ar' },
          { reply_id: 'src_oth_ar', title: '🌐 مصدر آخر', next_node_key: 'ask_city_ar' }
        ]}]
      }, position_x: 900, position_y: -400
    },
    ...
    // Location (Arabic)
    {
      ...
      config: {
        text: "دعنا نبدأ بالسؤال الأول. في أي بلد يقع عملك؟",
  ...
  nodes.push(
    {
      ...
      config: {
        ...
        sections: [{ title: 'الباقات', rows: [
          ...
          { reply_id: 'pkg_pro_ar', title: '🛍️ الاحترافية (Professional)', next_node_key: 'ask_timeline_ar' },
  ...
  // Let's insert the Arabic flow nodes
  ...
  if (langNodes && langNodes.length > 0) {
    ...
    const newConfig = {
      ...
      buttons: [
        { reply_id: 'lang_en', title: 'English', next_node_key: 'ask_customer_type' },
        { reply_id: 'lang_ar', title: 'العربية (Arabic)', next_node_key: 'ask_customer_type_ar' },
        { reply_id: 'lang_hi', title: 'हिंदी (Hindi)', next_node_key: 'ask_customer_type_hi' }
      ]
    };
...
addWelcomeArabicFlow().catch(console.error);
